...
const PrivateChatScreen = ({ route, navigation }) => {
  ...
  const setupPrivateMessageListeners = () => {
    // Message listener
    ChatService.addMessageListener((messageRoomId, message) => {
    ...
    ChatService.addOnlineUsersListener((data) => {
      if (data.roomId === roomId) {
        const isParticipantOnline = data.users?.some(u => u.id === participantId);
        setParticipantOnline(isParticipantOnline || false);
      }
    });
  };
  ...
  const loadPrivateMessages = () => {
    ...
    // Add participant info to messages for display
    const messagesWithParticipantInfo = roomMessages.map(message => ({
      ...message,
      isOwn: message.senderId === user?.id,
      senderName: message.senderId === user?.id ? user?.name : participantName,
      senderAvatar: message.senderId === user?.id ? user?.avatar : participantAvatar
    }));
    
    setMessages(messagesWithParticipantInfo);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (olderMessages.length > 0) {
        const messagesWithInfo = olderMessages.map(message => ({
          ...message,
          isOwn: message.senderId === user?.id,
          senderName: message.senderId === user?.id ? user?.name : participantName,
          senderAvatar: message.senderId === user?.id ? user?.avatar : participantAvatar
        }));
        setMessages(prev => [...messagesWithInfo, ...prev]);
      }
    } catch (error) {
  ...
  const handleClearChat = () => {
    Alert.alert(
      '대화 내용 삭제',
      '이 대화의 모든 내용을 삭제하시겠습니까?',
      [
        { text: '취소', style: 'cancel' },
        { text: '삭제', onPress: confirmClearChat, style: 'destructive' }
      ]
    );
  };
  ...
  const confirmClearChat = () => {
    setMessages([]);
    // Also clear from ChatService storage
  ...
  const handleBlockUser = () => {
    Alert.alert(
      '사용자 차단',
      `${participantName}님을 차단하시겠습니까?`,
      [
        { text: '취소', style: 'cancel' },
        { text: '차단', onPress: confirmBlockUser, style: 'destructive' }
      ]
    );
  };
    ...
    >
      ...
      <Appbar.Header style={{ backgroundColor: theme.colors.surface }}>
        ...
        <Appbar.Content 
          ...
          subtitle={participantOnline ? '온라인' : '오프라인'}
        />
        <Menu
          visible={showMenu}
        ...
        >
          <Menu.Item onPress={handleClearChat} title="대화 내용 삭제" />
          <Menu.Item onPress={handleBlockUser} title="사용자 차단" />
        </Menu>
      </Appbar.Header>
        ...
        ListEmptyComponent={() => (
          <View style={styles.emptyContainer}>
            <Text style={[styles.emptyText, { color: theme.colors.outline }]}>
              {participantName}님과의 대화를 시작해보세요
            </Text>
          </View>
        )}
